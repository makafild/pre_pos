...
namespace core\Packages\order\src\controllers;
 ...
 */
class OrderPackageController extends CoreController
{
    private $_fillable = [
        'payment_method_id',
        'discount',
    ...
    public function invoice_index(Request $request, $sort = "id", $order = "desc", $limit = 10)
    {
        ...
        $orders = $orders->filter($request->all(), OrderFilter::class)->orderBy($sort, $order)->jsonPaginate();
        return $orders;
    }
    ...
    {
        $orderIds = $this->prepareData($request, 'store');
    ...
    public function prepareData($request, $status, $orderData = null, $invoice = false)
    {
        ...
        if (auth('api')->user()['kind'] == 'customer' ) {
            $cities = City::pluck("id")->all();
        }
            ...
            foreach ($finalPromotions[Promotions::KIND_BASKET] as $finalPromotion) {
                foreach ($finalPromotion->awards as $award) {
                    ...
                    if ($status == 'update') {
                        ...
                        if (empty($detailUpdateInvoice->get()->toArray())) {
                            $detail->order_id = $orderData['id'];
                            $detail->save();
                        } else {
                            $detailUpdateInvoice->update($detail->toArray());
    ...
    public function changeStatus(ChangeOrderStatusRequest $request)
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($request->status['name'] != 'rejected') {
                event(new OrderSendToRobot($id));
            }
        }

        foreach ($companiesOrders as $companyOrders) {
            event(new ChangeStatusEvent($companyOrders, $request->status['name'], $request->text ?: ''));
        }
    ...
    public function payment_method_list()
    {
        ...
        $visitorPositions = PaymentMethod::where('company_id', $company_id)
            ->orderBy('id', 'desc')->jsonPaginate(10);
    ...
    public function getCountVisitVisitorInDays(Request $request)
    {
        ...
        $dayins=implode("'),('", $days['milady']);
       DB::insert("insert into ".$dbname." (date) values('".$dayins."')");
       //end db
       $sql_where="";
       if (auth('api')->user()->kind == 'company') {

        $sql_where="where u.company_id=".auth('api')->user()->company_id;
    }

       $sql="SELECT ((select COUNT(*) AS total FROM orders o WHERE o.visitor_id=v.id AND CAST(o.created_at as DATE) =t.date ) + (select COUNT(*) AS total FROM reason_for_not_visitings r WHERE r.visitor_id=v.id AND CAST(r.created_at as DATE)=t.date)) AS total
       ,t.date,v.id as id
       ,CONCAT(u.first_name,' ',u.last_name) AS name
        FROM `".$dbname."` t LEFT JOIN visitors v ON(1=1) LEFT JOIN users u ON (v.user_id=u.id) ".$sql_where."
     ...
     {
       $series[$result->id]['name']=$result->name;
       $series[$result->id]['data'][]=$result->total;
     }
    ...
    } public function getPercentCategory(Request $request)
    {
        ...
        if (auth('api')->user()->kind == 'company') {
            $companyId = auth('api')->user()->company_id;
            $sql=" and p.company_id=".auth('api')->user()->company_id;
        }
        $sql.=" and  d.created_at >='".$request->date_from."' AND d.created_at <='".$request->date_to."'";

        $total = Detail::with('Product');

       $constants= DB::select("SELECT c.constant_fa ,(SELECT COUNT(*) FROM product_user_category pc JOIN products p ON(p.id=pc.product_id) JOIN details d ON (d.product_id=p.id) WHERE pc.constant_id=c.id ".$sql.") AS total FROM constants c WHERE kind ='customer_category'");
         $total = Detail::whereBetween('created_at',$date)->count();
        ...
        foreach($constants as $constant){
            ...
            $result['products'][]=$constant->constant_fa;
            $result['percent'][]=($constant->total && $total)?round((($constant->total) ?$constant->total / $total : 0) * 100,2):0;
    ...
    public function getBestProductsSeller(Request $request)
    {
        ...
        $products=Product::select(['name_fa'
          ]);
        $products->withCount('Details')
        ->orderby('details_count','desc');
        if ($companyId) {
                $products->where('company_id', $companyId);
        }
         $products->whereBetween('created_at',$date);
         $products=$products->limit(5)->get();

         $total = Order::whereBetween('created_at',$date)->count();
        ...
        foreach($products as $product){
            $result['products'][]=$product->name_fa;
            $result['percent'][]=round((($product->details_count) ?$product->details_count / $total : 0) * 100,2);
