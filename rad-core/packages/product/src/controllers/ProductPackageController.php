...
class ProductPackageController extends CoreController
{
    ...
    public function index(Request $request, $limit = 10)
    {
        ...
        if ($request->has('paginate') && $request->get('paginate') == 'false') {
            return $products->filter($request->all());
        }

        return $products->filter($request->all())->orderBy('id','desc')->jsonPaginate($limit);
    }
    ...
    {
        $product->secureDelete($request->id, ['Details', 'ReportsSaleProductRoute', 'Labels', 'Barcodes', 'Visit', 'Promotions']);
    }
