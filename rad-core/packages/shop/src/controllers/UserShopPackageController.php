...
use function Siler\Functional\lfind;
...
class UserShopPackageController extends CoreController
{
    ...
    public function address_show($id)
    {
        ...
        $address = GeoAddress::with('city.Province', 'area', 'receiver')->where('user_id', auth('api')->user()->id)->find($id);
        return response($address);
    ...
    public function dates(Request $request)
    {
        ...
        $weekOfdays = [];
        for ($i = 1; $i <= 7; $i++) {
            $weekOfdays[] =  ['date' => date('Y-m-d', strtotime($date))];
            $date = date('Y-m-d', strtotime('+1 day', strtotime($date)));
        ...
        $tours = ShopTourDelivery::whereIn('id', $tours_id)->with('dates.times')->get();
        // return  $tours;
        ...
        foreach ($tours as $tour_index => $tour) {
            ...
            $result[] = ['vendor_id' => $tour->vendor_id, 'dates' => $weekOfdays];

            foreach ($tour['dates'] as $tour_dates) {

                foreach ($result as $result_index => $each) {

                    foreach ($each['dates'] as $days_index => $days) {

                        if (!empty($tour_dates['date']) && !empty($days['date']) && $days['date'] == $tour_dates['date']) {
                            $result[$result_index]['dates'][$days_index]['value']   = $tour_dates;
                        }
                    }
                }
            }
        ...
        //    return response(['data' => $weekOfdays])  ;
