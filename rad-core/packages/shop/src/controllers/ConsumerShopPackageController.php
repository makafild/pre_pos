...
use Core\Packages\shop\src\request\StoreRequest;
use Core\System\Http\Controllers\CoreController;
use Core\Packages\shop\src\request\UpdateRequest;


use Core\Packages\shop\src\request\DestroyRequest;
use Core\Packages\customer\src\request\ApproveStateRequest;
use Core\Packages\Consumer\src\request\StoreRequest as sreq;
use Core\Packages\customer\src\request\AddCustomerToRouterRequest;

class ConsumerShopPackageController extends CoreController
{
    ...
    public function index(Request $request, $order = "desc", $limit = 10)
    {
        ...
        $customer = Users::where('users.kind', Users::KIND_CONSUMER)
            ->select('users.*')
 ...
 return $customer->filter($request->all(), CustomerFilter::class)->get();
    ...
    public function update(UpdateRequest $request, $id)
    {
       ...
       // CrmSabz::_()->checkCorectData($request);
        ...
        // $updateCrm = CrmSabz::_()->storeCrm('update', 'panel', $request, $id, $customer->referral_id);
        // if ($updateCrm) {
        //     $customer->save();
        // }
    ...
    public function store(StoreRequest $request)
    {
       // CrmSabz::_()->checkCorectData($request);
        ...
        // $referralId = '';
        // $storeCrm = CrmSabz::_()->storeCrm('create', 'panel', $request, '');
        // if ($storeCrm) {
        //     $crmRegistered = 1;
        ...
        $customer->first_name = $request->first_name;
        $customer->kind = Users::KIND_CONSUMER;
        $customer->last_name = $request->last_name;
        ...
        // $customer->crm_registered = $crmRegistered;
        // if (!empty($referralId)) {
        ...
        foreach ($request->addresses as $address) {
            ...
            $addressEntity->lat = $address['lat'] ?? "";
            $addressEntity->long = $address['long'] ?? "";
            $addressEntity->city_id = $request->city;
            $addressEntity->area_id = $request->area;
            $addressEntity->User()->associate($customer);
