...
class OrderShopPackageController extends CoreController
{
    ...
    public function index(Request $request, $sort = "id", $order = "desc", $limit = 10)
    {
        ...
        if (auth('api')->user()->kind == "consumer") {
            $result = [];
            $grope_ids = Order::select('group_id')->where([['customer_id', auth('api')->user()->id], ['group_id', '!=', null]])->distinct()->get();


            foreach ($grope_ids as $group_id) {

                $order = Order::where('group_id', $group_id->group_id)->get();
                $price = array_sum($order->pluck('final_price')->toArray());
                $create_at = date($order->first()->created_at);
                $register = (count(array_unique($order->pluck('status')->toArray()) ) == 1 ) ? array_unique($order->pluck('status')->toArray())[0] : 'unknown';




                $result[] = ['price' => $price, 'create_at' => $create_at, 'status' => 'registered', 'data' => $order];
            }
            return $result;
